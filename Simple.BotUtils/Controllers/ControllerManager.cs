...
public class ControllerManager
{
    ...
    public T ExecuteFromText<T>(object context, string text)
    {
        ...
        var newArgs = new object[args.Length - 1 + 1];
        newArgs[0] = context;
    ...
    public T Execute<T>(string[] methodWithParameters)
    {
        ...
        {
            var arguments = new string[methodWithParameters.Length - 1];
    ...
    private T execute<T>(string method, object[] parameters)
    {
    ...
    private object[] convertParams(EndpointInfo info, MethodInfo methodInfo, object[] parameters)
    {
        ...
        var values = new object[paramInfo.Length];
        ...
        for (int i = 0; i < paramInfo.Length; i++)
        {
            ...
            else if (paramInfo[i].GetCustomAttributes(false).Any(a => a is ParamArrayAttribute))
            {
                ...
                {
                    var arr = new object[parameters.Length - pCount];
                    Array.Copy(parameters, pCount, arr, 0, arr.Length);
                    values[i] = arr;
                }
